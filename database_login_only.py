...
class ScheduleScreen(Screen):
    ...
    def add_schedule(self):
        ...
        if all([subject, building_name, room, day, time_start, time_end]):
            ...
            Snackbar(text="Schedule added!").open()
        else:
    ...
    def room_picker(self):
        ...
        if not building_name:
            Snackbar(text="Select building first").open()
            return
    ...
    def prepare_rooms_menu(self, building_name):
        ...
        if not rooms_list:
            Snackbar(text="No rooms found").open()
            return
...
class MapScreen(Screen):
    ...
    def menu_callback(self, day):
        ...
        Snackbar(text=f"Showing schedule for {day}").open()
    ...
    def show_markers_for_day(self, day):
        ...
        if not hasattr(schedule_screen, "table") or schedule_screen.table is None:
            Snackbar(text="No schedule table found").open()
            return
        ...
        if not buildings_with_classes:
            Snackbar(text=f"No classes found for {day}").open()
        else:
            Snackbar(text=f"Found {len(buildings_with_classes)} building(s)").open()
    ...
    def show_building_dialog(self, building_name, classes=None):
        if building_name not in VALID_ROOMS:
            Snackbar(text=f"Building {building_name} not found").open()
            return
